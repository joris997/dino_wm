...
from RL.policies import DinoRLFeatureExtractor, DinoRLStateEncoder
...
def make_env(step_in_real_env:bool=False, 
             average_pool:bool=False,
             plot_target:bool=True)->gym.Env:
    ...
    if plot_target:
        ...
        for i in range(world_model.local_hist):
            ...
            ax[i].axis('off')
        fig.savefig(f"outputs/RL/{current_date}/{current_time}/target_latent_state.png", 
    ...
    env = Monitor(gym.wrappers.TimeLimit(env, max_episode_steps=6))
    return env
...
if __name__ == "__main__":
    ...
    os.makedirs(f"outputs/RL/{current_date}/{current_time}/videos", exist_ok=True)
    ...
    # env = make_env(0)
    env = DummyVecEnv([lambda: make_env(step_in_real_env=False,average_pool=False)])
    ...
    env = VecVideoRecorder(env,
                        f"outputs/RL/{current_date}/{current_time}/videos",
                        record_video_trigger=lambda x: x % 10000 == 0,
                        video_length=200)

    # policy_kwargs = dict(net_arch=dict(pi=[512,512,512], 
    #                                    qf=[512,512,512]))
    # policy_kwargs = dict(
    ...
    # )
    policy_kwargs = dict(
        features_extractor_class=DinoRLFeatureExtractor,
        features_extractor_kwargs=dict(
            hist_size=3,        # must match env
            embed_dim=384,
    ),
)
    ...
    if policy == "TD3":
        model = TD3("MlpPolicy", env, action_noise=u_noise, verbose=0,
                    gamma=0.995, buffer_size=int(1e5), learning_rate=1e-4,
        ...
        model = SAC("MlpPolicy", env, action_noise=u_noise, verbose=0,
                    gamma=0.995, buffer_size=int(1e4),#learning_rate=1e-3,
                    policy_kwargs=policy_kwargs, device='cuda',
                    tensorboard_log=f"outputs/RL/{current_date}/{current_time}/runs")
    ...
    model.learn(total_timesteps=config['total_timesteps'],progress_bar=True,
                ...
                model_save_path=f"outputs/RL/{current_date}/{current_time}/models",
                verbose=2)
    )
    model.save(f"outputs/RL/{current_date}/{current_time}/models/{policy}_latent_dynamics")
    wandb.finish()
