...
class _RolloutDataset(torch.utils.data.Dataset): # pylint: disable=too-few-public-methods
    def __init__(self, root, transform, buffer_size=200, train=True): # pylint: disable=too-many-arguments
        ...
        for r in root:
            self._files += [
                join(r, sd, ssd)
                for sd in listdir(r) if isdir(join(r, sd))
                for ssd in listdir(join(r, sd))]
        # now shuffle the elements in the list
...
class RolloutClassifierObservationDataset(_RolloutDataset): # pylint: disable=too-few-public-methods
    def _data_per_sequence(self, data_length):
        return data_length
    
    def _get_data(self, data, seq_index):
        # data['infos'][seq_index] is a dictionary of np.True_ and np.False_,
        # convert it to an integer 0 or 1
        return (
            self._transform(data['observations'][seq_index]),
            data['infos'][seq_index]
        )